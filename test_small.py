...
def train_features_extraction(synthetic=False, name='test', batch_size=80, epochs=1,
    ...
    if synthetic:
        ...
        data_gen = modelnet.ModelnetData(pointcloud_size=CLOUD_SIZE, clusterize=False, dev_fraction=0.05)
    ...
    if LOAD_MODEL:
        features_vars = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope="end-to-end")
...
#             exit()
            ...
            #for batch_in_epoch_idx, (clouds, labels) in enumerate(tqdm(data_gen.generate_representative_batch_for_train(
                        instances_number=INSTANCES_NUMBER, shuffle_points=SHUFFLE_CLOUDS,
                        shuffle_clouds=False, jitter_points=True, rotate_pointclouds=False,
                        rotate_pointclouds_up=ROTATE_CLOUDS_UP, sampling_method=SAMPLING_METHOD, augment=80), total=BATCHES)):
