...
def conv2d(input, conv2d_vars, batchnorm_vars,
    ...
    with tf.variable_scope(conv2d_vars.scope) as sc:
        stride_h, stride_w = stride
        outputs = tf.nn.conv2d(input, conv2d_vars.kernel, [1, stride_h, stride_w, 1], padding=padding)
        outputs = tf.nn.bias_add(outputs, conv2d_vars.biases)
...
def fully_connected(input, fullyconn_vars, batchnorm_vars,
    ...
    with tf.variable_scope(fullyconn_vars.scope) as sc:
        outputs = tf.matmul(input, fullyconn_vars.weights)
        outputs = tf.nn.bias_add(outputs, fullyconn_vars.biases)
...
class BatchNormVars(object):
    def __init__(self, scope):
        with tf.variable_scope(scope) as sc:
            self.scope = scope
            self.beta = tf.get_variable('beta', initializer=np.array(0.0, dtype=np.float32))
            self.gamma = tf.get_variable('gamma', initializer=np.array(1.0, dtype=np.float32))

def batch_norm(inputs, batchnorm_vars, is_training, moments_dims, bn_decay):
    with tf.variable_scope(batchnorm_vars.scope) as sc:
